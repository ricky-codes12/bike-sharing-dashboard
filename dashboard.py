...
with tabs[3]:
    ...
    if filtered_df.empty:
        ...
        # === 1️⃣ Marker Cluster ===
        marker_cluster = MarkerCluster(name="Marker Cluster").add_to(m)
        ...
        for _, row in filtered_df.iterrows():
            folium.CircleMarker(
                ...
                radius=5,
                popup=f"📍 Stasiun: {row['station_name']}<br>🚲 Penyewaan: {row['cnt_day']}",
                color=marker_color,
                fill=True,
                fill_opacity=0.6
        ...
        # === 2️⃣ Heatmap ===
        heat_data = filtered_df[['lat', 'lon', 'cnt_day']].values.tolist()
        HeatMap(heat_data, name="Heatmap Penyewaan", radius=15, blur=20, max_zoom=13).add_to(m)
